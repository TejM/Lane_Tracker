...
def stabilize(file_name, output_name):
    ...
    output_video = cv2.VideoWriter(
        output_name, cv2.cv.CV_FOURCC(*'SVQ3'), 30.0, (cols, rows))
...
class LineCluster:
    ...
    def __init__(self, y_min, y_max, (x1, y1), (x2, y2), allowed_error=5000):
        ...
        self.bottom_line.append(
            np.int(np.float((self.y_min - intercept)) / np.float(slope)))
        self.top_line.append(
            np.int(np.float((self.y_max - intercept)) / np.float(slope)))
...
def track_lanes(image):
    ...
    if lines is None:
        return image
    lines = lines[0]
    clustered_lines = average_line_variable_cluster(
        lines, (x_min, y_min, x_max, y_max))
    ...
    if deviation < 0:
        ...
        print "Correct"
    mid_line = [((r_line[0][0] + l_line[0][0])/2, (r_line[0][1] + l_line[0][1])/2),
                ((r_line[1][0]+l_line[1][0])/2, (r_line[1][1] + l_line[1][1])/2)]
